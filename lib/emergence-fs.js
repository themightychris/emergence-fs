...
// TODO: deduplicate createFileFrom* functions
efs.prototype.createFileFromTmp = function(parent, handle, tmpPath, hash, size, callback) {
	var me = this
	...
	function _moveFile() {
		fs.rename(tmpPath, hashPath, function(error) {
			if(error) {
				return callback(error);
			}
			
			console.log('moved tmp file', handle, tmpPath, hashPath);
			_insertRecord();
		});
	}
	...
	// get last revision to check if save is needed and track ancestorId
	me.getFile(parent, handle, function(error, lastFile) {
	
		if(!error && lastFile) {
			// test if file is identical
			if(lastFile.sha1 == hash && lastFile.remoteId == null && lastFile.status == 'normal') {
				if(lastFile.size != size) {
					throw 'Possible hash collision on '+hash;
				}
				
				fs.unlink(tmpPath); // async call and we don't care when it finishes

				// no substantial change from last revision, call it a success and do nothing
				return callback();
			}
			
			ancestorId = lastFile.id;
		}
		
		// check if object with same hash is already stored
...
efs.prototype.createFileFromData = function(parent, handle, data, callback) {
	var me = this
