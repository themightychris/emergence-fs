...
*/
					...
					,updateMergingCollections: function(next) {
						...
						flatiron.common.async.mapSeries(mergePlan.update, function(job, next) {
							...
							// TODO: limit all this stuff to lastJob
							flatiron.common.async.series({
						...
						flatiron.common.async.series({
							...
							,insertPhantoms: function(next) {
								...
								con.query(
									...
									+' SELECT null, ? + lft, ? + rgt, handle, "local", null, null, ?'
									+' FROM _cm'
									+' WHERE lft > 0'
								...
								con.query(
									...
									+',('
										+'SELECT'
											+' id'
											+',('
												+'SELECT MAX(cr2.id)' // assumes new ids are always inserted at right edges and not moved
...
function _getTreeMergePlan(con, srcCollection, destCollection, callback) {
	...
	console.log('_getMergePlan', srcCollection, destCollection);
	...
	// if the destination is a leaf, no crawling for collision with existing nodes need be done
	if(destCollection.lft+1 == destCollection.rgt) {
		mergeQueue[0].displacement = srcCollection.rgt - srcCollection.lft - 1;
		return callback(null, mergeQueue, createQueue);
	}
...
*/
							...
							// TODO: if this is the only place lft and rgt get used, replace with displacement
/*
...
*/
		...
		,function(error) {
			console.log('finished merge planning loop, error=', error);
			...
			for(i = createQueue.length - 1; i >= 0; i--) {
				...
				job.destLft = parentUpdateJob.displacedRgt;
				job.destRgt = job.destLft + displacement - 1;
				console.log('Set lft/rgt on job', job);
								
				while(parentUpdateJob) {
					parentUpdateJob.displacement += displacement;
					parentUpdateJob.displacedRgt += displacement;
					
					// bump earlier children right
					for(j = 0; j < parentUpdateJob.newChildren.length; j++) {
						parentUpdateJob.newChildren[j].destLft += displacement;
						parentUpdateJob.newChildren[j].destRgt += displacement;
					}
					
					parentUpdateJob = updateMap[parentUpdateJob.destParentId];
				}
				
				updateMap[job.srcParentId].newChildren.unshift(job);
			}
			
			console.log('mapped createQueue', createQueue);
